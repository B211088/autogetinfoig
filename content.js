...
function extractProfileInfo() {
  ...
  if (!username) {
    throw new Error(
      "Không tìm thấy username. Vui lòng mở trang profile Instagram."
    );
  }
    ...
    listItems.forEach((li) => {
      ...
      if (numbers && numbers.length > 0) {
        ...
        if (
          ...
          (text.includes("posts") && posts === "0")
        ) {
          posts = num;
        } else if (
          (text.includes("follower") ||
            text.includes("followers") ||
            text.includes("người theo dõi")) &&
          followers === "0"
        ) {
          followers = num;
        } else if (
          (text.includes("following") || text.includes("đang theo dõi")) &&
          following === "0"
        ) {
          following = num;
        }
...
function fillBirthdayFields() {
  ...
  console.log(
    "Random date: " + randomDay + "/" + randomMonth + "/" + randomYear
  );
